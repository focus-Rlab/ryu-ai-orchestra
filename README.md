...
- Version: 0.6.0-draft
- Status: Stage 2開始前の役割・言語・ロードマップ・引き継ぎ整合修正中
- 現在の中核エージェント: Raphael
...
14. 完了時は、参照した正本、変更点、検証、未解決事項、承認待ちを報告し、重要な現在地を`PROJECT_HANDOFF.md`へ反映する。
...
4. 既存責任との重複を監査する。
5. 調査、設計、実装、検証を行う。
6. 問題発生時は根本原因分析と横断監査を行い、一般化した再発防止を設計する。
7. 小変更は監査理由を残してmainへ直接コミットできる。重要変更は一つの専用ブランチとDraft PRで提出する。
8. 正本との整合性、セキュリティ、テスト結果を確認する。
9. 隆之介の承認後に重要変更をmainへ反映する。
10. 重要な新決定は、関連する正本・完全版・対応表・PROJECT_HANDOFFへ同期する。
11. マージ済みの作業ブランチは削除し、常時存在する作業ブランチを原則1〜2本に抑える。
